...
export const GameOverModal: React.FC<GameOverModalProps> = ({
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white rounded-3xl p-6 sm:p-8 border border-moss-200 shadow-xl shadow-moss-100 space-y-6 text-center animate-in fade-in zoom-in-95 duration-200">
      ...
      {/* Main Score Box */}
      <div className="bg-moss-50/60 rounded-3xl p-6 border border-moss-100 shadow-inner space-y-3">
        <span className="text-xs font-bold text-slate-500">最終獲得スコア (加点総合計)</span>
        <div className="text-4xl sm:text-5xl font-black text-amber-600 tracking-tight">
          {totalScore.toLocaleString()} <span className="text-xl font-bold text-amber-500">pt</span>
        </div>

        {/* Rank Title Pill */}
        <div className="inline-block mt-2">
          <div className="px-4 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-sm sm:text-base font-black text-amber-700">
            {rankBadge.title}
          </div>
        </div>
        <p className="text-xs text-slate-500 max-w-md mx-auto">{rankBadge.desc}</p>

        {/* 3 Metric Cards */}
        <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-3">
          <div className="bg-white rounded-xl p-3 border border-moss-100 text-center">
            <div className="flex items-center justify-center gap-1 text-[11px] font-semibold text-emerald-600 mb-1">
          ...
          <div className="bg-white rounded-xl p-3 border border-moss-100 text-center">
            <div className="flex items-center justify-center gap-1 text-[11px] font-semibold text-orange-500 mb-1">
          ...
          <div className="bg-white rounded-xl p-3 border border-moss-100 text-center">
            <div className="flex items-center justify-center gap-1 text-[11px] font-semibold text-amber-600 mb-1">
      ...
      <div className="bg-moss-50/60 rounded-2xl p-4 border border-moss-100 text-left space-y-2">
        <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider">
          出題履歴と配点内訳
        </h4>
        <div className="divide-y divide-moss-100 text-xs">
        ...
        {!isSaved ? (
          <form onSubmit={handleSaveToRanking} className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 bg-white border border-moss-200 rounded-xl px-3 py-2 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-moss-500 focus:ring-1 focus:ring-moss-500"
            />
            <button
              id="btn-save-ranking"
              type="submit"
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
        ...
        <button
          ...
          type="button"
          onClick={onOpenLeaderboard}
          className="w-full sm:w-auto px-6 py-3 rounded-2xl bg-white hover:bg-moss-50 text-slate-700 border border-moss-200 font-semibold text-sm flex items-center justify-center gap-2 transition-all"
        >
          <Trophy className="w-4 h-4 text-amber-500" />
